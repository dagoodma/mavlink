...
class 
def slugslogconverter(opts, args)
...
if __name__ == "__main__":
    ...
    parser.add_option("--no-timestamps",dest="notimestamps", action='store_true', help="Log doesn't align on timestamps")
    parser.add_option("--align-timestamps",dest="align_timestamps", default="
    ...
    if len(args) < 1:
        parser.error("Usage: mavlogdump.py [options] <LOGFILE>")

    slugslogconverter(opts, args)
